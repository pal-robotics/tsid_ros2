...
controller_interface::CallbackReturn CartesianVelocityController::on_configure(
  ...
  /*VMO: In this point we make a list of joints to remove from the model (in
   tiago case wheel joints and end effector joints) These are actually the
   joints that we don't passs to the controller*/
  ...
  /* VMO: if we need to check the mass of the model to verify that it is more or
  less realistic IPE: 18.21225 arms+torso ~= what we expected*/
  RCLCPP_INFO(get_node()->get_logger(), "Total mass according to the model %f",
  ...
  for (int i = 0; i < v_min.size(); i++) {
    RCLCPP_INFO(get_node()->get_logger(), "position: %f", v_max[i]);
  }
  ...
  for (auto ee : ee_names_) {
    ...
    task_ee_.push_back(new tsid::tasks::TaskCartesianVelocity(
    ...
    double ee_weight = 1;
    int ee_priority = 1;
    formulation_->addMotionTask(*task_ee_[ee_id_[ee]], ee_weight, ee_priority,
                                transition_time);
  }
...
controller_interface::CallbackReturn CartesianVelocityController::on_activate(
  ...
  for (const auto &joint : joint_names_) {
    ...
    v0.tail(robot_wrapper_->nv() - 6)[model_.getJointId(joint) - 2] =
  ...
  for (auto ee : ee_names_) {
    H_ee_0_[ee_id_[ee]] = robot_wrapper_->framePosition(formulation_->data(),
                                                        model_.getFrameId(ee));
...
CartesianVelocityController::update(const rclcpp::Time & /*time*/,
...
void CartesianVelocityController::setVelCallback(
  ...
  task_ee_[ee_id_[ee]]->setReference(sample_vel_ee);
  std::cout << "setVelCallback" << std::endl;
  RCLCPP_INFO(get_node()->get_logger(), "Desired velocity: %f %f %f %f %f %f",
...
PLUGINLIB_EXPORT_CLASS(tsid_controllers::CartesianVelocityController,
