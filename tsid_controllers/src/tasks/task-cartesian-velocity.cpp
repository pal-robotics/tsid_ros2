#include "tsid/math/utils.hpp"
#include "tsid_controllers/tasks/task-cartesian-velocity.hpp"
#include "tsid/robots/robot-wrapper.hpp"

namespace tsid
{
namespace tasks
{
using namespace std;
using namespace math;
using namespace trajectories;
using namespace pinocchio;

TaskCartesianVelocity::TaskCartesianVelocity(
  const std::string & name, RobotWrapper & robot,
  const std::string & frameName, const double & dt)
: TaskMotion(name, robot),
  m_frame_name(frameName),
  m_constraint(name, 6, robot.nv()),
  m_ref(6, 6),
  dt_(dt)
{
  PINOCCHIO_CHECK_INPUT_ARGUMENT(
    m_robot.model().existFrame(frameName),
    "The frame with name '" + frameName + "' does not exist");
  m_frame_id = m_robot.model().getFrameId(frameName);

  m_v_ref.setZero();
  m_a_ref.setZero();
  m_M_ref.setIdentity();
  m_wMl.setIdentity();
  m_p_error_vec.setZero(6);
  m_v_error_vec.setZero(6);
  m_p.resize(12);
  m_v.resize(6);
  m_p_ref.resize(12);
  m_v_ref_vec.resize(6);
  m_Kp.setZero(6);
  m_Kd.setZero(6);
  m_Ki.setZero(6);
  m_a_des.setZero(6);
  m_J.setZero(6, robot.nv());
  m_J_rotated.setZero(6, robot.nv());

  m_mask.resize(6);
  m_mask.fill(1.);
  setMask(m_mask);

  m_local_frame = true;
}

void TaskCartesianVelocity::setMask(math::ConstRefVector mask)
{
  TaskMotion::setMask(mask);
  int n = dim();
  m_constraint.resize(n, (unsigned int)m_J.cols());
  m_p_error_masked_vec.resize(n);
  m_v_error_masked_vec.resize(n);
  m_p_error = Motion::Zero();
  v_err_prev = Motion::Zero();
  m_drift_masked.resize(n);
  m_a_des_masked.resize(n);
}

int TaskCartesianVelocity::dim() const {return (int)m_mask.sum();}

const Vector & TaskCartesianVelocity::Kp() const {return m_Kp;}

const Vector & TaskCartesianVelocity::Kd() const {return m_Kd;}

const Vector & TaskCartesianVelocity::Ki() const {return m_Ki;}


void TaskCartesianVelocity::Kp(ConstRefVector Kp)
{
  PINOCCHIO_CHECK_INPUT_ARGUMENT(
    Kp.size() == 6,
    "The size of the Kp vector needs to equal 6");
  m_Kp = Kp;
}

void TaskCartesianVelocity::Kd(ConstRefVector Kd)
{
  PINOCCHIO_CHECK_INPUT_ARGUMENT(
    Kd.size() == 6,
    "The size of the Kd vector needs to equal 6");
  m_Kd = Kd;
}

void TaskCartesianVelocity::Ki(ConstRefVector Ki)
{
  PINOCCHIO_CHECK_INPUT_ARGUMENT(
    Ki.size() == 6,
    "The size of the Ki vector needs to equal 6");
  m_Ki = Ki;
}

void TaskCartesianVelocity::setReference(TrajectorySample & ref)
{
  m_ref = ref;
  TSID_DISABLE_WARNING_PUSH
  TSID_DISABLE_WARNING_DEPRECATED
    PINOCCHIO_CHECK_INPUT_ARGUMENT(
    ref.pos.size() == 6, "The size of the reference vector needs to be 6");
  // m_M_ref.translation(ref.pos.head<3>());
  // m_M_ref.rotation(MapMatrix3(&ref.pos(3), 3, 3));
  TSID_DISABLE_WARNING_POP
    m_v_ref = ref.getValue();
  m_a_ref = Motion(ref.getDerivative());
  first = true;
}

void TaskCartesianVelocity::setReference(const TrajectorySample & ref)
{
  TSID_DISABLE_WARNING_PUSH
  TSID_DISABLE_WARNING_DEPRECATED
  TSID_DISABLE_WARNING_POP
    m_ref = ref;
  first = true;

}

const TrajectorySample & TaskCartesianVelocity::getReference() const {return m_ref;}

const Vector & TaskCartesianVelocity::position_error() const
{
  return m_p_error_masked_vec;
}

const Vector & TaskCartesianVelocity::velocity_error() const
{
  return m_v_error_masked_vec;
}

const Vector & TaskCartesianVelocity::position() const {return m_p;}

const Vector & TaskCartesianVelocity::velocity() const {return m_v;}

const Vector & TaskCartesianVelocity::position_ref() const {return m_p_ref;}

const Vector & TaskCartesianVelocity::velocity_ref() const {return m_v_ref_vec;}

const Vector & TaskCartesianVelocity::getDesiredAcceleration() const
{
  return m_a_des_masked;
}

Vector TaskCartesianVelocity::getAcceleration(ConstRefVector dv) const
{
  return m_constraint.matrix() * dv + m_drift_masked;
}

Index TaskCartesianVelocity::frame_id() const {return m_frame_id;}

const ConstraintBase & TaskCartesianVelocity::getConstraint() const
{
  return m_constraint;
}

void TaskCartesianVelocity::useLocalFrame(bool local_frame)
{
  m_local_frame = local_frame;
}

const ConstraintBase & TaskCartesianVelocity::compute(
  const double, ConstRefVector,
  ConstRefVector, Data & data)
{
  SE3 oMi;
  Motion v_frame;
  m_robot.framePosition(data, m_frame_id, oMi);
  m_robot.frameVelocity(data, m_frame_id, v_frame);
  m_robot.frameClassicAcceleration(data, m_frame_id, m_drift);

  // @todo Since Jacobian computation is cheaper in world frame
  // we could do all computations in world frame
  m_robot.frameJacobianLocal(data, m_frame_id, m_J);

  // Transformation from local to world
  m_wMl.rotation(oMi.rotation());


  m_v_error =
    m_v_ref - m_wMl.act(v_frame);      // vel err in local world-oriented frame


  m_a_error = (m_v_error - v_err_prev ) / dt_;    // acc err in local world-oriented frame

  m_p_error += m_v_error * dt_;


  // desired acc in local world-oriented frame
  m_drift = m_wMl.act(m_drift);

  m_a_des = m_Kp.cwiseProduct(m_v_error.toVector()) + m_Ki.cwiseProduct(m_p_error.toVector()) +
    m_Kd.cwiseProduct(m_a_error.toVector()) +
    m_v_ref.toVector() + m_robot.model().gravity.toVector();

  // Use an explicit temporary `m_J_rotated` here to avoid allocations.
  m_J_rotated.noalias() = m_wMl.toActionMatrix() * m_J;
  m_J = m_J_rotated;


  m_v_error_vec = m_v_error.toVector();
  m_v_ref_vec = m_v_ref.toVector();
  m_v = v_frame.toVector();
  v_err_prev = m_v_error;


  int idx = 0;
  for (int i = 0; i < 6; i++) {
    if (std::abs(m_mask(i) - 1.0) > std::numeric_limits<double>::epsilon()) {continue;}

    m_constraint.matrix().row(idx) = m_J.row(i);
    m_constraint.vector().row(idx) = (m_a_des ).row(i);
    m_a_des_masked(idx) = m_a_des(i);
    m_drift_masked(idx) = m_drift.toVector()(i);
    m_p_error_masked_vec(idx) = m_p_error_vec(i);
    m_v_error_masked_vec(idx) = m_v_error_vec(i);


    idx += 1;
  }

  pos_prev = oMi;
  v_prev = v_frame;
  m_a_prev = m_drift;

  first = false;

  return m_constraint;
}
}  // namespace tasks
}  // namespace tsid
