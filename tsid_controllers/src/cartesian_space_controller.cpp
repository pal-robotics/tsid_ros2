  ...
  const rclcpp_lifecycle::State & prev_state)
{
  ...
  for (const auto &ee : ee_names_) {
    ...
    TsidPositionControl::formulation_->addMotionTask(*task_ee_[ee_id_[ee]], ee_weight, ee_priority, transition_time);
  }
  ...
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  for (auto ee : ee_names_) {
    H_ee_0_[ee_id_[ee]] =
      robot_wrapper_->framePosition(TsidPositionControl::formulation_->data(), TsidPositionControl::model_.getFrameId(ee));
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  auto h_ee_ =
    TsidPositionControl::robot_wrapper_->framePosition(TsidPositionControl::formulation_->data(), TsidPositionControl::model_.getFrameId(ee_names_[0]));
   ...
   if (get_node()->get_current_state().id() == lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
    for (int i = 0; i < msg->ee_name.size(); i++) {
      if (std::find(ee_names_.begin(), ee_names_.end(), msg->ee_name[i]) == ee_names_.end()) {
        ...
        auto h_ee_ =
          TsidPositionControl::robot_wrapper_->framePosition(TsidPositionControl::formulation_->data(), TsidPositionControl::model_.getFrameId(ee_names_[i]));
    ...
    for (auto ee : ee_names_) {
      if (std::find(msg->ee_name.begin(), msg->ee_name.end(), ee) == msg->ee_name.end()) {
        auto h_ee_ =
          TsidPositionControl::robot_wrapper_->framePosition(TsidPositionControl::formulation_->data(), TsidPositionControl::model_.getFrameId(ee));
